...
public class SMCSamplerExperiments implements Runnable
{
	...
	@Option public ArrayList<InferenceMethod> methods = list(Arrays.asList(InferenceMethod.PMMHNC, InferenceMethod.SMC));
	...
	public static enum InferenceMethod
	{
		...
		SMC4GTRIGamma {
			...
			public TreeDistancesProcessor doIt(SMCSamplerExperiments instance, double iterScale, UnrootedTree goldut, String treeName)
			{
				...
				options.verbose = instance.verbose;
				//		        options.maxNGrow = 0;				
				double[] subsRates=instance.generator.subsRates;   // six parameters of substitutions:rAC,rAG,rAT,rCG,rGT,rCT
				double[] statFreqs=instance.generator.stationaryDistribution;   // stationary state frequencies. pi_A, pi_C, pi_G, pi_T				
				double alpha=instance.generator.alpha;       // shape parameter in the Gamma distribution
				double pInv=instance.generator.pInv;        // the proportion of invariant sites 							
				int nCategories=4;
				int dataRepeatN=nCategories; 
		...
		PGS4GTRIGammaBF {
			...
			{
				ParticleFilterOptions options = new ParticleFilterOptions();
				options.nParticles = instance.nParticlesEachStep; // generator.nTaxa*(generator.nTaxa-1)*20;  //(int) (iterScale * instance.nThousandIters * 1000);
				//options.nThreads = instance.nThreads;
				...
				double alpha=Sampling.nextDouble(instance.mainRand,0.1,0.9);       // shape parameter in the Gamma distribution						
				if(instance.betterStartVal){
					double[] subsRatesTmp=new double[subsRates.length];
					for(int i=0;i<subsRates.length;i++) subsRatesTmp[i]=instance.generator.subsRates[i]*10;
					subsRates=Dirichlet.sample(instance.mainRand,subsRatesTmp);			
					double[] subStatFreqsTmp=new double[statFreqs.length];
					for(int i=0;i<statFreqs.length;i++) subStatFreqsTmp[i]=instance.generator.stationaryDistribution[i]*10;
					statFreqs=Dirichlet.sample(instance.mainRand,subStatFreqsTmp);
					alpha=Sampling.nextDouble(instance.mainRand, Math.max(instance.generator.alpha-0.1, 0),Math.max(instance.generator.alpha+0.1, 1.0));
				}
				...
				while(i<nPGS)
				{
					...
					if(i>nPGSburnin) pg.setProcessTree(true);
		...
		ANNEALING {
					...
					double iterScale, UnrootedTree goldut, String treeName)
			{
				...
				AnnealingKernel ppk = new AnnealingKernel(
