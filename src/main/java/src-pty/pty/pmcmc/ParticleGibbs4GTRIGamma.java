...
public class ParticleGibbs4GTRIGamma {
	...
	public ParticleGibbs4GTRIGamma(Dataset dataset0,ParticleFilterOptions options,TreeDistancesProcessor tdp,
	...
	public ParticleGibbs4GTRIGamma(Dataset dataset0, ParticleFilterOptions options,  TreeDistancesProcessor tdp, RootedTree initrt, double[] subsRates, double[] statFreqs, double alpha, double pInv,TreeTopologyProcessor trTopo)
	{
		this.dataset=dataset0;
		this.options = options;
		this.tdp = tdp;
	...
	public void next(Random rand)
	{
		...
		if((iter % sampleTreeEveryNIter) == 0)
		{
			...
			LazyParticleKernel kernel = (isGS4Clock?new PriorPriorKernel(init):new NCPriorPriorKernel(init));		
